...
impl PhotonBench {
    ...
    async fn open_table(config: Arc<Args>, env: &Photon) -> Arc<Table<Photon>> {
    ...
    async fn cleanup(&mut self) {
        if let Some(table) = self.table.take() {
            if let Ok(_t) = Arc::try_unwrap(table) {
                // t.close().await
                // https://github.com/photondb/photondb/issues/174
            }
        }
    }
}
